...
public class RobotConfigurator {
    ...
    /*public static TecbotEncoder buildEncoder(TecbotSpeedController speedController, int a, int b) {
        if (speedController != null) {
            ...
            if (speedController.getType() == TypeOfMotor.CAN_SPARK_BRUSHLESS) {
                return new TecbotEncoder(speedController);
            }
    ...
    public static DoubleSolenoid buildDoubleSolenoid(int[] ports) {
        System.out.println(ports[0] + " " + ports[1] + " " + ports[2] + " " );
    ...
    public static Solenoid buildSingleSolenoid(int highSolenoidPort, int lowSolenoidPort) {
        return new Solenoid(lowSolenoidPort, TecbotConstants.PNEUMATICS_MODULE_TYPE, highSolenoidPort);
    }
